...
from copaw.zero_token.config import is_zero_token_web_provider_id
...
class TokenRecordingModelWrapper(ChatModelBase):
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        # openclaw-zero-token: web/shim stacks ignore parallel_tool_calls; forwarding it
        # can break single-tool-call providers. Strip only for zero-token-web provider.
        if is_zero_token_web_provider_id(self._provider_id):
            kwargs = dict(kwargs)
            kwargs.pop("parallel_tool_calls", None)
            kwargs.pop("parallelToolCalls", None)

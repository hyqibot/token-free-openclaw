...
_t = time.perf_counter()
from ..utils.packaged_runtime import configure_packaged_runtime_defaults  # noqa: E402

_record("..utils.packaged_runtime", time.perf_counter() - _t)
...
def cli(ctx: click.Context, host: str | None, port: int | None) -> None:
    """CoPaw CLI."""
    # Nuitka 入口未必加载 copaw.__main__，须在此也执行，否则 COPAW_FLOW_TRACE 未注入、打点全关。
    configure_packaged_runtime_defaults()
    # 测试诊断：追踪谁触发了 ``-m`` 子进程（仅在设置 COPAW_TRACE_M=1 时启用）
    from ..utils.trace_m_flag import install_trace_m_flag_from_env, log_cli_entry_probe

    install_trace_m_flag_from_env()
    log_cli_entry_probe(invoked_subcommand=ctx.invoked_subcommand)
    ...
    # Packaged/copaw.exe UX: double-click (no args) should behave like `copaw desktop`.
    # Also handy for `python -m copaw` with no subcommand.
    if ctx.invoked_subcommand is None:
        cmd = ctx.command.get_command(ctx, "desktop")
        if cmd is None:
            raise click.ClickException("Failed to load default command: desktop")
        ctx.invoke(cmd)

...
当你需要“真正执行动作”（打开网页/下载/读写文件/运行命令/调用技能）时，必须触发工具调用，而不是只给建议。

- **优先**：如果当前模型支持结构化工具调用（function/tool calling），按系统提供的工具接口直接触发工具调用。
- **兼容模式**：如果当前模型不支持结构化工具调用，也必须用下面的 tag 格式输出工具调用，系统会解析并执行：
...
- 只输出合法 JSON（双引号、无注释）。
- `arguments` 必须是对象（`{}`）。
- 需要连续调用多个工具时，按顺序输出多个 `<tool_call>...</tool_call>`。
- 除了必要的 `<tool_call>`，不要夹杂与执行无关的长篇解释，等工具结果返回后再总结。

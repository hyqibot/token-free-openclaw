...
>
> **Skills 扩展** — 内置定时任务、PDF/Office 处理、新闻摘要等；**Windows** 下另有按名打开单个桌面/开始菜单快捷方式（`open_desktop_shortcuts`）、原生窗口 UI 自动化脚本（`native_window_control`，需 `pip install pywinauto`）、以及 exe 资源包放置约定（`exe_bundle`）。自定义技能自动加载，无绑定。通过 Skills 决定 CoPaw 能做什么。
>
...
# 将控制台构建产物复制到包目录（备注，实测 npm build后直接生成到src/copaw/console/目录下了）

mkdir -p src/copaw/console
cp -R console/dist/. src/copaw/console/

PowerShell 的 Copy-Item（cp）在目标目录已存在时不会像 Unix 那样合并覆盖，会报 “项已存在”。应先删除旧 dist 再复制，或使用 robocopy。

已经按上面方式在 D:\CoPaw-main 里执行完成：先删掉 src\copaw\console\dist，再把 console\dist 整目录复制过去。

原因简述：
在 PowerShell 里 cp -R 实际是 Copy-Item -Recurse。目标路径里已经有 dist、dist\assets 等目录时，再往里拷同名项会触发 ResourceExists，不会像部分 Unix 环境那样直接覆盖合并。

以后你可以用：

Set-Location D:\CoPaw-main
Remove-Item -Recurse -Force src\copaw\console\dist -ErrorAction SilentlyContinue
Copy-Item -Recurse -Force console\dist src\copaw\console\
或用镜像同步（只同步 dist，会删目标里多出来的文件）：

robocopy console\dist src\copaw\console\dist /MIR
（robocopy 在目标目录不存在时会自动创建。）
